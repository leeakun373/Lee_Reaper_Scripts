...
-- 如果items_to_process只有一个item，且该item在cursor左侧，需要找到cursor前的最后一个item
if #items_to_process == 1 and init_item_start >= cursor_pos then
    -- 查找同一轨道上cursor位置前的最后一个item
    ...
    for i = track_item_count - 1, 0, -1 do
        ...
        if item then
            ...
            if item_end <= cursor_pos then
                ...
                init_item_start = reaper.GetMediaItemInfo_Value(item, "D_POSITION")
...
if #items_to_process == 1 then
    -- 关键修复：检查cursor是否在item范围内，如果cursor在item右侧（会拉长item），则不处理
    if cursor_pos > init_item_end then
        return -- cursor在item右侧，不应该trim（会拉长item）
    end
    
    if init_item_start >= cursor_pos then
        return -- 没有找到合适的item
    end
    ...
    -- 多个items时，过滤掉不在cursor左侧的items，并且cursor不能在item右侧（防止拉长）
    local filtered_items = {}
    for _, item in ipairs(items_to_process) do
        local item_start = reaper.GetMediaItemInfo_Value(item, "D_POSITION")
        local item_length = reaper.GetMediaItemInfo_Value(item, "D_LENGTH")
        local item_end = item_start + item_length
        -- cursor必须在item左侧（在item范围内），不能在item右侧
...
if max_end == 0 then
    max_end = init_item_end
    min_start = init_item_start
end

if min_start >= cursor_pos then
    return
end

local init_diff = max_end - cursor_pos
...
for i, item in ipairs(items_to_process) do
    ...
    -- 只有item在cursor左侧时才需要修剪
    if item_start < cursor_pos and (diff >= init_diff - 0.0001 or diff > 0 or (#items_to_process > 1 and i == 1)) then
        ...
        -- Trim Right时：淡入保持不变（不应该调整淡入）
        -- 淡入设置已经在trim后保持不变，无需额外处理
        
        -- Trim Right时：淡出长度保持不变（保持原有的淡出长度）
        -- 参考nvk的实现：让REAPER内置命令处理，然后手动恢复淡出长度
        local original_fadeout_len = fadeout_len
        
        -- 如果使用内置命令，内置命令会自动处理，我们需要恢复淡出长度
        if track_visible then
            -- 内置命令已经执行，我们需要恢复淡出长度
            if original_fadeout_len > 0.0001 then
                -- 确保淡出长度不超过item长度
                local restored_fadeout_len = math.min(original_fadeout_len, item_length)
