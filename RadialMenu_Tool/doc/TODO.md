...
## Phase 1: Infrastructure & Data (基础设施与数据)

### 配置系统

- [ ] **Setup `index.lua` to check for ReaImGui installation**

  - 检查 `reaper.ImGui_CreateContext` 是否存在
  - 如果不存在，显示错误消息并退出
  - 设置模块搜索路径
- [ ] **Implement `config_manager.lua` to create a default JSON if none exists**
...
- [ ] **Define the JSON structure (Sectors -> Slots -> Data)**
...
- [ ] **Implement `json.lua` encode/decode functions**
  - 实现 `encode(data)` 函数
...
## Phase 2: The Wheel (UI & Math) (轮盘界面与数学)

### 数学工具

- [ ] **Implement `math_utils.lua`: Function `GetMouseAngleAndDistance()`**
...
- [ ] **Implement `gui/wheel.lua`: Use `ImDrawList` to draw 6 static sectors**
...
- [ ] **Add logic to highlight the sector based on mouse angle**
...
- [ ] **Implement `gui/styles.lua`: Define colors, fonts, and spacing**
  - 定义颜色常量（背景、文本、悬停等）
  - 定义尺寸常量（半径、边框宽度等）
...
- [ ] **Implement `main_runtime.lua`: Main loop with defer**
...
## Phase 3: The Submenu & Interaction (子菜单与交互)

### 列表视图

- [ ] **Implement `gui/list_view.lua`: Show a window/child next to the active sector**

  - 实现 `draw_submenu(ctx, sector_data)` 绘制子菜单
  - 实现 `draw_slot_item(ctx, slot, index)` 绘制单个插槽项
  - 实现 `calculate_submenu_position(...)` 计算子菜单位置
  - 确保子菜单不超出屏幕边界
- [ ] **Populate list from the Config data**
...
- [ ] **Implement `logic/actions.lua`: Wrappers to execute Reaper Command IDs**
...
## Phase 4: Logic Implementation (业务逻辑实现)

### FX 引擎

- [ ] **Implement `logic/fx_engine.lua`: `SmartAddFX(name)` function (Item vs Track detection)**
...
- [ ] **Implement Drag & Drop logic (from ImGui item to Reaper window)** ⚠️
  - ⚠️ **警告**: 这是技术难度最高的功能
  - ReaImGui 的拖拽主要用于 ImGui 窗口内部
  - 跨窗口拖拽到 Reaper 原生界面可能无法直接实现
  - **备选方案（推荐）**: 使用"点击-点击"模式
    1. 点击轮盘上的 FX
    2. 鼠标变成特殊图标（如插头形状）
    3. 用户点击目标轨道/Item
    4. 使用 `reaper.GetTrackFromPoint()` 捕捉目标并完成挂载
  - 如果实现拖拽：
    - 在 `list_view.lua` 中实现 `setup_drag_source(ctx, slot)`
    - 实现拖拽数据的设置
    - 实现拖拽预览
    - 研究 ImGui 到 Reaper 窗口的数据传递

### 搜索功能

- [ ] **Implement Fuzzy Search in the Settings menu**
  - 实现 `logic/search.lua`：
...
- [ ] **Implement `main_settings.lua`: Settings Editor UI**
  - 实现 `init()` 和 `loop()` 主循环
